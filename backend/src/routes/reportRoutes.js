...
router.get('/CC125-hourly', async (req, res) => {
  const { date } = req.query;
  try {
    const report = await getHourlyCC125Report(date);
    res.json(report);
  } catch (error) {
    console.error('Ошибка при формировании отчета:', error);
    res.status(500).json({ error: 'Не удалось сформировать отчет' });
...
router.get('/CC168-hourly', async (req, res) => {
  const { date } = req.query;
  try {
    const report = await getHourlyCC168Report(date);
    res.json(report);
  } catch (error) {
    console.error('Ошибка при формировании отчета:', error);
    res.status(500).json({ error: 'Не удалось сформировать отчет' });
